...
const einlesen=(path)=>{ 
  ...
  {JSONtools.lesen("./userData.json", function(x,data){
    resolve(data)
  })
  })
  }
...
function validateBedarfE(bedarfE){
    const schemabedarfE = joi.object({
        unit: joi.string().valid('prozent').required(),  
        kcal:joi.number().min(0).required(),
          protein:joi.number().min(0).required(),
          fett:joi.number().min(0).required(),
          gesFett:joi.number().min(0).required(),
          ungesFett:joi.number().min(0).required(),
          carbs:joi.number().min(0).required(),
          zucker:joi.number().min(0).required()
     })

     return schemabedarfE.validate(bedarfE)
}
  function validateBedarf(bedarf) {
    const schemabedarf=joi.object({ 
    kcal:joi.number().min(0).required(),
    protein:joi.number().min(0).required(),
    fett:joi.number().min(0).required(),
    gesFett:joi.number().min(0).required(),
    ...
    return schemabedarf.validate(bedarf)
